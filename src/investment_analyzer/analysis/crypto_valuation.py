"""V12.71 crypto strategic/tactical scoring with explicit coverage and verdicts."""
...
class CryptoAnalysis:
    ...
    strategic_verdict:str|None=None; tactical_verdict:str|None=None
...
class CryptoDecisionEngine:
    ...
    def analyze(self, *, market_cap=None,volume_24h=None,circulating_supply=None,max_supply=None,total_supply=None,volatility=None,max_drawdown=None,onchain_score=None,valuation_score=None,momentum=None,trend=None,exchange_flow=None,smart_money=None):
        ...
        ss,sc=calc(svals,self.STRATEGIC); ts,tc=calc(tvals,self.TACTICAL); warnings=[]
        ...
        sv='BUY' if ss>=70 and sc>=50 else 'SELL' if ss<40 else 'HOLD'; tv='BUY' if ts>=70 and tc>=40 else 'SELL' if ts<40 else 'HOLD'
        return CryptoAnalysis(ss,ts,sc,tc,{"strategic":svals,"tactical":tvals},warnings,sv,tv)

"""Executable trade-plan calculations used by the real analysis pipeline."""
...
def build_trade_plan(*, price, fair_value=None, bear_value=None, bull_value=None,
    ...
    bear = _num(bear_value)
    bull = _num(bull_value)
    ...
    if stop_loss is None:
        reasons.append("Stop Loss no disponible")
    if target_1 is None:
        reasons.append("Target 1 no disponible")
    if target_2 is None:
        reasons.append("Target 2 no disponible")
    if mos is None or mos < float(min_mos_pct):
        reasons.append(f"Margin of Safety inferior a {float(min_mos_pct):.1f}%")
    if rr is None or rr < float(min_rr):
        reasons.append(f"R/R no disponible o inferior a {float(min_rr):.1f}")
    if units <= 0:
        reasons.append("Tamaño de posición no permitido por riesgo/capital")

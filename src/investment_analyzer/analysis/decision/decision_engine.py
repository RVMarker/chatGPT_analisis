...
class DecisionEngine:
    ...
    def _normalize_weight(weight, total):
        # Do not round the normalized float. The exact IEEE-754 representation
        # of 2/3 must remain 0.6666666666666666 rather than 0.6666666667;
        # presentation rounding belongs in as_dict(), not in decision math.
    ...
    def _weighted(self, data: Mapping[str, object], weights: Mapping[str, float]):
        ...
        if not available_items:
            for item in items: item.weight = 0.0
            return None, items, ["Ningún componente disponible; veredicto N/D"]
        total_weight = sum(item.weight for item in available_items)
        for item in items:
            item.weight = self._normalize_weight(item.weight, total_weight) if item.available else 0.0
        score = sum(Decimal(str(item.score)) * Decimal(str(item.weight)) for item in available_items)
        return round(float(score), 2), items, []
    ...
    @staticmethod
    def confidence(provider_quality, freshness, consistency, completeness, technical_data_quality=None, coverage=100.0, valuation_quality=None):
        core=[(provider_quality,.30),(freshness,.20),(consistency,.30),(completeness,.20)]
        available=[(max(0.0,min(100.0,float(v))),w) for v,w in core if v is not None]
        if not available: return 0.0
        total_weight=sum(w for _,w in available); quality=sum(v*w for v,w in available)/total_weight; evidence_coverage=total_weight
        technical=DecisionEngine._quality_score(technical_data_quality)
        if technical is not None: quality=quality*.90+technical*.10
        valuation=DecisionEngine._quality_score(valuation_quality)
        if valuation is not None: quality=quality*.90+valuation*.10
    ...
    def evaluate(self, strategic_scores, tactical_scores, confidence_inputs, strengths=None, red_flags=None, contextual=None):
        strategic_total, strategic_items, strategic_warnings=self.strategic(strategic_scores); tactical_total,tactical_items,tactical_warnings=self.tactical(tactical_scores)
        strategic_coverage,tactical_coverage=self._coverage(strategic_items),self._coverage(tactical_items); coverage=round((strategic_coverage+tactical_coverage)/2.0,2)
        valuation_quality=confidence_inputs.get("valuation_quality")
        args=(confidence_inputs.get("provider_quality"),confidence_inputs.get("freshness"),confidence_inputs.get("consistency"),confidence_inputs.get("completeness"),confidence_inputs.get("technical_data_quality"))
        base_confidence=self.confidence(*args,100,valuation_quality); confidence=self.confidence(*args,coverage,valuation_quality)
        context={}
        for key,value in (contextual or {}).items():
            score=self._score(value)
            if score is not None: context[key]=score
        strategic_sufficient=strategic_total is not None and strategic_coverage>=self.MIN_DECISION_COVERAGE; tactical_sufficient=tactical_total is not None and tactical_coverage>=self.MIN_DECISION_COVERAGE
        ...
        if valuation_quality is not None and str(valuation_quality).upper()!="HIGH": flags.append(f"Calidad de valoración {str(valuation_quality).upper()}: reduce confianza, pero no altera el score ni el veredicto")
        ...
        return DecisionResult(strategic_total,tactical_total,self._decision(strategic_total),self._decision(tactical_total),confidence,strategic_items,tactical_items,list(dict.fromkeys(flags)),list(strengths or []),context,coverage,base_confidence,strategic_coverage,tactical_coverage,strategic_sufficient,tactical_sufficient)

    @staticmethod
    def print_summary(result):
        strategic=f"{result.strategic_score:.2f}/100" if result.strategic_score is not None else "N/D"; tactical=f"{result.tactical_score:.2f}/100" if result.tactical_score is not None else "N/D"
        print("="*80); print("DECISION ENGINE V12"); print("="*80); print(f"Estratégico (años): {result.strategic_decision} | {strategic}"); print(f"Táctico (semanas):  {result.tactical_decision} | {tactical}"); print(f"Confianza:          {result.confidence:.1f}%"); print(f"Cobertura señales:  {result.data_coverage:.1f}%")

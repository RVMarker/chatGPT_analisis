...
class DecisionEngine:
    ...
    def _weighted(self, data: Mapping[str, object], weights: Mapping[str, float]):
        ...
        for key, weight in weights.items():
            ...
            score = (
                self._score(raw.get("score"))
                if isinstance(raw, Mapping) and available
                else self._score(raw)
                if available
                else None
            )
            items.append(
                ScoreComponent(
                    ...
                    explanation=(
                        ...
                        else "NO DISPONIBLE — no participa en el promedio"
                    ),
                )
            )
    ...
    def _coverage(items: list[ScoreComponent]) -> float:
        return (
            round(100.0 * sum(item.available for item in items) / len(items), 2)
            if items
            else 0.0
        )
    ...
    def evaluate(
        ...
        strategic_total, strategic_items, strategic_warnings = self.strategic(
            strategic_scores
        )
        tactical_total, tactical_items, tactical_warnings = self.tactical(
            tactical_scores
        )
        ...
        if not strategic_sufficient and strategic_total is not None:
            flags.append(
                ...
                "el veredicto estratégico es una señal, no una conclusión robusta"
            )
        if not tactical_sufficient and tactical_total is not None:
            flags.append(
                f"Cobertura táctica insuficiente: {tactical_coverage:.1f}%; "
                "el veredicto táctico es una señal, no una conclusión robusta"
            )
        if coverage < 100:

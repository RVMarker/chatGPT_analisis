...
class DecisionAnalyzer:
    ...
    def build(self, fundamental_score: float, dcf_score: float, comparables_score: float, macro_score: float,
              ...
              counter_thesis: list[str] | None = None) -> dict[str, Any]:
        result = self.engine.evaluate(
            strategic_scores={"fundamental": fundamental_score, "valuation": dcf_score, "risk": risk_score},
            tactical_scores={"technical": technical_score, "sentiment": sentiment_score, "smart_money": smart_money_score},
            confidence_inputs={"provider_quality": provider_quality, "freshness": freshness, "consistency": consistency,
                               "completeness": completeness, "valuation_quality": valuation_quality},
        ...
        return {"result": result,
                ...
                "confidence": result.confidence, "contextual": result.contextual, "counter_thesis": counter_thesis or [],
                "strengths": result.strengths, "red_flags": result.red_flags}
    ...
    def build_from_context(self, context: Any) -> dict[str, Any]:
        ...
        return self.build(_score(fundamentals), _score(valuation_source), _score(comparables), _score(macro), _score(risk),
                          ...
                          _merge_lists(fundamentals, risk, key="red_flags"), _list(risk, "counter_thesis"))
...
def _value(obj, key, default): return _number(obj.get(key) if isinstance(obj, Mapping) else getattr(obj, key, default), default)

"""V12.84 final decision quality gate.
Separates asset quality from whether the current entry is attractive.
"""
...
class DecisionQualityGate:
    def evaluate(self, *, strategic_verdict: str|None, tactical_verdict: str|None,
                 ...
                 data_coverage: float|None = None, min_rr: float = 2.0,
                 min_mos: float = 0.10) -> dict[str, Any]:
        reasons=[]
        ...
        valuation_ok=upside is None or upside>=float(min_mos)
        rr_ok=risk_reward is None or float(risk_reward)>=float(min_rr)
        ...
        if not valuation_ok: reasons.append('Margen de seguridad insuficiente')
        if not rr_ok: reasons.append(f'R/R inferior a {min_rr:.1f}')
        ...
        if not reasons and score_ok and valuation_ok and rr_ok and coverage_ok:
            operation='COMPRAR'
        elif strategic_verdict in {'VENDER','REDUCIR'} and not score_ok:
            operation='VENDER'
        else:
            operation='ESPERAR'
        return {'status':'OK','operation':operation,'reasons':reasons,'entry':p,'fair_value':fair_value,'upside_to_fair_value_pct':round(upside*100,2) if upside is not None else None,'margin_of_safety_pct':round(mos*100,2) if mos is not None else None,'risk_reward':round(float(risk_reward),2) if risk_reward is not None else None,'strategic_verdict':strategic_verdict,'tactical_verdict':tactical_verdict,'score':score,'data_coverage':data_coverage,'thresholds':{'min_score':65,'min_rr':min_rr,'min_margin_of_safety':min_mos,'min_coverage':70}}

...
class SpecializedDispatcher:
    def __init__(self, *, stock=None, etf=None, reit_fibra=None, crypto=None, bond=None):
        self.stock=stock or StockAnalyzer(); self.etf=etf or ETFAnalyzer(); self.reit_fibra=reit_fibra or REITFibraAnalyzer(); self.crypto=crypto or CryptoAnalyzer(); self.bond=bond or BondAnalyzer()
    @staticmethod
    ...
    def analyze(self,asset_type:str,symbol:str,payload:dict[str,Any]|None=None)->dict[str,Any]:
        ...
        if a=='ETF': return {'asset_type':a,'analysis':self.etf.analyze(symbol,p)}

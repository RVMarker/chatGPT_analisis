...
class FinancialAnalysisIntegrator:
    ...
    def run(self, statements: FinancialStatements, price: PriceData, *, growth_rates: list[float] | None = None,
            wacc: float | None = None, terminal_growth: float | None = None, net_debt: float | None = None,
            asset_type: str | None = None, reit_required_yield: float = 0.09, reit_growth: float = 0.03) -> IntegratedFinancialAnalysis:
        if price.current <= 0:
        ...
        if price.shares_outstanding_source == "yahoo_fast_info_reconciled":
            valuation_warnings.append("Shares outstanding reconciliadas contra market cap/precio de Yahoo; "
                                      f"escala aplicada: {price.shares_outstanding_scale:g}")
        elif price.shares_outstanding_source == "market_cap/current_price":
            valuation_warnings.append("Shares outstanding no disponibles en Yahoo; derivadas de market cap/precio del mismo proveedor")
        ...
        if is_reit and price.shares_outstanding:
            ...
            if ffo is not None:
                reit = self.reit_valuation.calculate(
                    ffo=float(ffo), shares_outstanding=float(price.shares_outstanding), current_price=float(price.current),
                    required_yield=reit_required_yield, growth=reit_growth, source_quality=source_quality,
                    affo=cashflow.affo_official, distribution=cashflow.dividends_paid,
                    distribution_period=cashflow.dividends_paid_period, property_value=balance.property_value,
                    net_debt=debt, ebitda=income.ebitda, interest_expense=income.interest_expense,
                )
            ...
            if fcf is not None:
                dcf: DCFResult = self.valuation.calculate(
                    fcf_base=float(fcf), growth_rates=growth_rates, wacc=float(wacc), terminal_growth=float(terminal_growth),
                    net_debt=debt, shares_outstanding=price.shares_outstanding, current_price=price.current,
                )
            ...
            valuation = {"available": False, "score": None, "warnings": [
        ...
        return IntegratedFinancialAnalysis(fundamental=fundamental.as_dict(), valuation=valuation,

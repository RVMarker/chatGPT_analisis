...
class FinancialAnalysisIntegrator:
    ...
    def _derive_wacc(cls, statements: FinancialStatements, price: PriceData, risk_free_rate=None):
        ...
        debt = max(0.0, float(statements.balance.long_term_debt or 0.0)); market_cap = max(0.0, float(price.market_cap or 0.0)); total_capital = market_cap + debt
        ...
        interest = abs(float(statements.income.interest_expense or 0.0)); pretax = float(statements.income.pretax_income or 0.0); net_income = float(statements.income.net_income or 0.0)
        tax_rate = 1.0 - (net_income / pretax) if pretax > 0 and 0 <= net_income <= pretax else 0.21
        tax_rate = max(0.0, min(0.35, tax_rate)); cost_debt = interest / debt if interest > 0 else rf + 0.015; cost_debt = max(0.02, min(0.12, cost_debt)); ew = market_cap / total_capital; dw = debt / total_capital
    ...
    def run(self, statements: FinancialStatements, price: PriceData, *, growth_rates=None, wacc=None, terminal_growth=None, net_debt=None, asset_type=None, reit_required_yield=.09, reit_growth=.03, risk_free_rate=None):
        ...
        if is_reit and price.shares_outstanding:
            ...
            if terminal_growth is None and growth_rates is not None:
                terminal_growth = min(self.DEFAULT_TERMINAL_GROWTH, max(-0.01, float(growth_rates[-1]) * 0.5)); terminal_growth = max(-0.01, min(0.03, terminal_growth)); wacc_assumptions["terminal_growth_method"]="conservative_fade_of_historical_FCF_growth"
            ...
            if fcf is not None and growth_rates is not None and wacc is not None and terminal_growth is not None and price.shares_outstanding:
                ...
                valuation=base.as_dict(); valuation["available"]=base.fair_value_per_share is not None; valuation["score"]=REITValuationEngine._score(base.margin_of_safety) if base.margin_of_safety is not None else None; valuation["model"]="FCFF_DCF"; valuation["assumption_source"]="historical_provider_data_plus_explicit_model_defaults"; valuation["wacc_details"]=wacc_assumptions; valuation["historical_fcf"]=list(getattr(cashflow,"historical_fcf",[])); valuation["bear_case"]=bear.fair_value_per_share; valuation["base_fair_value"]=base.fair_value_per_share; valuation["bull_case"]=bull.fair_value_per_share; valuation["fair_value_low"]=bear.fair_value_per_share; valuation["fair_value_high"]=bull.fair_value_per_share; valuation["scenario_assumptions"]={"bear":{"growth_delta":-0.03,"wacc_delta":0.015,"terminal_delta":-0.005},"bull":{"growth_delta":0.02,"wacc_delta":-0.010,"terminal_delta":0.005}; valuation_warnings.extend(assumption_warnings); valuation_warnings.extend(base.warnings)
            else:
                missing=[]

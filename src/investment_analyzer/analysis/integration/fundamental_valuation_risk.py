...
class FinancialAnalysisIntegrator:
    ...
    def _derive_wacc(cls, statements: FinancialStatements, price: PriceData, risk_free_rate=None):
        rf=float(risk_free_rate) if risk_free_rate is not None else cls.DEFAULT_RISK_FREE; beta=float(price.beta) if price.beta and price.beta>0 else 1.0; cost_equity=rf+beta*cls.DEFAULT_EQUITY_RISK_PREMIUM
        debt=max(0.0,float(statements.balance.long_term_debt or 0.0)); market_cap=max(0.0,float(price.market_cap or 0.0)); total=market_cap+debt
        if total<=0 or debt<=0: return max(.06,min(.18,cost_equity)),{"risk_free_rate":rf,"beta":beta,"equity_risk_premium":cls.DEFAULT_EQUITY_RISK_PREMIUM,"cost_of_equity":cost_equity,"debt_weight":0.0,"equity_weight":1.0,"method":"CAPM_equity_only"}
        interest=abs(float(statements.income.interest_expense or 0.0)); pretax=float(statements.income.pretax_income or 0.0); net_income=float(statements.income.net_income or 0.0); tax_rate=1.0-net_income/pretax if pretax>0 and 0<=net_income<=pretax else .21; tax_rate=max(0.0,min(.35,tax_rate)); cost_debt=interest/debt if interest>0 else rf+.015; cost_debt=max(.02,min(.12,cost_debt)); ew,dw=market_cap/total,debt/total; wacc=ew*cost_equity+dw*cost_debt*(1-tax_rate)
        return max(.06,min(.18,wacc)),{"risk_free_rate":rf,"beta":beta,"equity_risk_premium":cls.DEFAULT_EQUITY_RISK_PREMIUM,"cost_of_equity":cost_equity,"pre_tax_cost_of_debt":cost_debt,"tax_rate":tax_rate,"debt_weight":dw,"equity_weight":ew,"method":"CAPM_WACC"}

    def _scenarios(self, fcf,growth_rates,wacc,terminal_growth,debt,shares,price):
        base=self.valuation.calculate(fcf,growth_rates,wacc,terminal_growth,debt,shares,price); bear=self.valuation.calculate(fcf,[max(-.08,g-.03) for g in growth_rates],min(.25,wacc+.015),max(.005,terminal_growth-.005),debt,shares,price); bull=self.valuation.calculate(fcf,[min(.18,g+.02) for g in growth_rates],max(terminal_growth+.015,wacc-.010),min(.035,terminal_growth+.005),debt,shares,price); return base,bear,bull
    ...
    def run(self, statements: FinancialStatements, price: PriceData, *, growth_rates=None,wacc=None,terminal_growth=None,net_debt=None,asset_type=None,reit_required_yield=.09,reit_growth=.03,risk_free_rate=None):
        ...
        fundamental=self.fundamental.calculate(statements); balance,income,cashflow=statements.balance,statements.income,statements.cashflow; debt=net_debt if net_debt is not None else float(balance.long_term_debt or 0)-float(balance.cash or 0); risk=self.risk.calculate(statements,market_value_equity=price.market_cap); valuation_warnings=[]
        ...
        if is_reit and price.shares_outstanding:
            ffo=cashflow.ffo_official if cashflow.ffo_official is not None else cashflow.ffo_proxy; source_quality="FFO_OFFICIAL" if cashflow.ffo_official is not None else "FFO_PROXY"
            if ffo is not None:
                reit=self.reit_valuation.calculate(ffo=float(ffo),shares_outstanding=float(price.shares_outstanding),current_price=float(price.current),required_yield=reit_required_yield,growth=reit_growth,source_quality=source_quality,affo=cashflow.affo_official,distribution=cashflow.dividends_paid,distribution_period=cashflow.dividends_paid_period,distribution_source=cashflow.distribution_source,property_value=balance.property_value,net_debt=debt,ebitda=income.ebitda,interest_expense=income.interest_expense); valuation=reit.as_dict(); valuation.update({"model":"FFO_CAPITALIZATION","ffo_proxy":cashflow.ffo_proxy,"ffo_official":cashflow.ffo_official,"affo_official":cashflow.affo_official,"assumptions":{"required_yield":reit_required_yield,"growth":reit_growth},"shares_outstanding_raw":price.shares_outstanding_raw,"shares_outstanding_source":price.shares_outstanding_source,"shares_outstanding_scale":price.shares_outstanding_scale}); valuation_warnings.extend(reit.warnings)
            ...
            if fcff is not None and growth_rates is not None and wacc is not None and terminal_growth is not None and price.shares_outstanding:
                ...
                valuation={"available":False,"score":None,"model":"FCFF_DCF","warnings":["Valuation no ejecutada: faltan datos trazables: "+", ".join(missing)]}; valuation_warnings.extend(assumption_warnings)
        strengths=list(dict.fromkeys(fundamental.strengths+risk.strengths)); red_flags=list(dict.fromkeys(fundamental.red_flags+risk.red_flags+valuation_warnings+valuation.get("warnings",[]))); return IntegratedFinancialAnalysis(fundamental=fundamental.as_dict(),valuation=valuation,risk=risk.as_dict(),strengths=strengths,red_flags=red_flags)

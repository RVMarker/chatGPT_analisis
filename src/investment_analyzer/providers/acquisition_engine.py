"""V12.47 acquisition with ETF and REIT/FIBRA enrichment."""
...
class MultiProviderAcquisitionEngine:
    def __init__(self,router=None,normalizer=None,health=None,etf_enricher=None,reit_fibra_enricher=None):
        self.router=router or DataAcquisitionRouter(); self.normalizer=normalizer or ProviderNormalizer(); self.health=health or ProviderHealthManager(); self.etf_enricher=etf_enricher or ETFEnricher(); self.reit_fibra_enricher=reit_fibra_enricher or REITFibraEnricher()
    def acquire(self,*,symbol,asset_type,fetchers:dict[str,Callable[[str],Any]]):
        ...
        if asset=="ETF":
            ...
            keys=("expense_ratio","benchmark","aum","holdings","tracking_difference","tracking_error")
        elif asset in {"REIT","FIBRA"}:
            enriched=self.reit_fibra_enricher.enrich(provider_payloads)
            keys=tuple(self.reit_fibra_enricher.ALIASES)
        else: keys=()
        if enriched:
            for key in keys:

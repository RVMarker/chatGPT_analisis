"""V12.36 end-to-end multi-asset investment pipeline with acquisition."""
...
class PipelineResult:
    identity:dict[str,Any]; classification:dict[str,Any]; route:dict[str,Any]; acquisition:dict[str,Any]; consensus:dict[str,dict[str,Any]]; analysis:dict[str,Any]; decision:dict[str,Any]; quality:dict[str,Any]; warnings:list[str]
    def as_dict(self): return asdict(self)

class InvestmentPipeline:
    def __init__(self,classifier=None,identity_registry=None,consensus=None,data_router=None,acquisition=None):
        self.classifier=classifier or AssetClassifier(); self.identity=identity_registry or InstrumentIdentityRegistry(); self.consensus=consensus or ProviderConsensus(); self.router=data_router or DataAcquisitionRouter(); self.acquisition=acquisition or MultiProviderAcquisitionEngine(self.router)
    @staticmethod
    ...
    def run(self,*,symbol,asset_type=None,isin=None,country=None,exchange=None,currency=None,provider_symbols=None,aliases=(),provider_metadata=None,consensus_data=None,analysis=None,strategic_score=None,strategic_coverage=0,tactical_score=None,tactical_coverage=0,data_quality=100.0,fetchers=None):
        ...
        acquisition=self.acquisition.acquire(symbol=symbol,asset_type=final_asset,fetchers=fetchers or {})
        consensus_input=consensus_data or {}
        if not consensus_input:
            consensus_input={field:[(x["provider"],x["value"]) for x in values] for field,values in acquisition.fields.items()}
        consensus=self.consensus.evaluate_batch(consensus_input,critical_fields=route["required_fields"])
        blocked=[f for f,r in consensus.items() if not r.vote_allowed]; missing=acquisition.missing_required
        cq=sum(r.quality_score for r in consensus.values())/len(consensus) if consensus else 0.0
        completeness=100.0*(1-len(missing)/len(route["required_fields"])) if route["required_fields"] else 100.0
        quality=min(float(data_quality),cq,completeness) if consensus else min(float(data_quality),completeness)
        warnings=[]
        ...
        return PipelineResult(ident.as_dict(),classification.as_dict(),route,acquisition.as_dict(),{k:r.as_dict() for k,r in consensus.items()},analysis or {},decision,{"data_quality":round(quality,2),"consensus_quality":round(cq,2),"completeness":round(completeness,2),"blocked_fields":blocked,"missing_required":missing},warnings)

"""V12.26 end-to-end investment analysis pipeline."""
...
class PipelineResult:
    identity:dict[str,Any]; classification:dict[str,Any]; consensus:dict[str,dict[str,Any]]; analysis:dict[str,Any]; decision:dict[str,Any]; quality:dict[str,Any]; warnings:list[str]
    def as_dict(self): return asdict(self)

class InvestmentPipeline:
    def __init__(self,classifier=None,identity_registry=None,consensus=None):
        self.classifier=classifier or AssetClassifier(); self.identity=identity_registry or InstrumentIdentityRegistry(); self.consensus=consensus or ProviderConsensus()
    @staticmethod
    ...
    def run(self,*,symbol,asset_type=None,isin=None,country=None,exchange=None,currency=None,provider_symbols=None,aliases=(),provider_metadata=None,consensus_data=None,analysis=None,strategic_score=None,strategic_coverage=0,tactical_score=None,tactical_coverage=0,data_quality=100.0):
        ...
        return PipelineResult(ident.as_dict(),classification.as_dict(),{k:r.as_dict() for k,r in consensus.items()},analysis or {},decision,{"data_quality":round(quality,2),"consensus_quality":round(cq,2),"blocked_fields":blocked},warnings)

"""V12.41 end-to-end multi-asset investment pipeline with ETF analysis."""
...
class InvestmentPipeline:
    def __init__(self,classifier=None,identity_registry=None,consensus=None,data_router=None,acquisition=None,etf_analyzer=None):
        self.classifier=classifier or AssetClassifier(); self.identity=identity_registry or InstrumentIdentityRegistry(); self.consensus=consensus or ProviderConsensus(); self.router=data_router or DataAcquisitionRouter(); self.acquisition=acquisition or MultiProviderAcquisitionEngine(self.router); self.etf_analyzer=etf_analyzer or ETFAnalyzer()
    @staticmethod
    ...
    def run(self,*,symbol,asset_type=None,isin=None,country=None,exchange=None,currency=None,provider_symbols=None,aliases=(),provider_metadata=None,consensus_data=None,analysis=None,strategic_score=None,strategic_coverage=0,tactical_score=None,tactical_coverage=0,data_quality=100.0,fetchers=None):
        ...
        specialized={}; specialized_warnings=[]
        if final_asset=="ETF":
            canonical_payload={k:(v[0]["value"] if v else None) for k,v in acquisition.fields.items()}
            etf=self.etf_analyzer.analyze(symbol,canonical_payload); specialized={"etf":etf.as_dict()}; specialized_warnings=etf.warnings or []

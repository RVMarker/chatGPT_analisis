...
class AnalysisPipeline:
    def __init__(self,providers,modules,financial_adapter=None,financial_loader=None,asset_loader=None,decision_module=None,technical_analyzer=None,sentiment_engine=None,smart_money_engine=None,actionable_adapter=None,asset_classifier=None,specialized_dispatcher=None,data_normalizer=None):
        self.providers=providers; self.modules=modules; self.asset_loader=asset_loader or AssetLoader(); self.asset_classifier=asset_classifier or AssetClassifier(); self.specialized_dispatcher=specialized_dispatcher; self.data_normalizer=data_normalizer or AssetDataNormalizer()
    ...
    def run(self,ticker:str)->AnalysisContext:
        asset=self.asset_loader.load(ticker); classification=self.asset_classifier.classify(asset.symbol,asset); context=AnalysisContext(asset=asset)
        ...
        snapshot=self.financial_loader.load(asset.symbol); context.price=snapshot.price; context.financials=snapshot.financials
        ...
        if self.specialized_dispatcher is not None:
            normalized=self.data_normalizer.normalize({'price':context.price,'financials':context.financials,'technical':context.technical,'metadata':context.metadata})
            normalized.update({'momentum':context.metadata.get('smart_money',{}).get('momentum'),'trend':context.technical.get('score') if isinstance(context.technical,Mapping) else None})
            context.metadata['normalized_asset_data']=normalized
            context.metadata['specialized_analysis']=self.specialized_dispatcher.analyze(classification.asset_type,asset.symbol,normalized)

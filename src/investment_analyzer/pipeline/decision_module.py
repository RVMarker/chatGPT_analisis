"""Pipeline adapter for V11/V12 DecisionEngine with one confidence source of truth."""
...
class DecisionModule:
    ...
    def run(self, context):
        ...
        technical=self._score(context.technical); sentiment=self._score(context.sentiment); smart_money=self._score(metadata.get("smart_money"))
        provider_data=metadata.get("data_providers",{}) or {}
        validation=metadata.get("provider_validation",{}) or {}

        # One source of truth: ConfidenceEngine owns usable coverage and confidence.
        ...
        evidence_confidence=confidence_result.confidence if confidence_result else 80.0
        evidence_coverage=confidence_result.coverage if confidence_result else 100.0
        blocked=list(confidence_result.blocked) if confidence_result else []
        missing=list(confidence_result.missing) if confidence_result else []
        ...
        # Engine calculates score; confidence is overwritten by the evidence engine only.
        result=self.engine.evaluate(
            ...
            confidence_inputs={"provider_quality":provider_quality,"freshness":freshness,"consistency":evidence_confidence,"completeness":completeness,"technical_data_quality":technical_quality,"valuation_quality":valuation_quality},
        ...
        result.data_coverage=evidence_coverage
        result.confidence=evidence_confidence
        result.base_confidence=evidence_confidence
        result.missing_factors.extend([f"{x}: sin evidencia utilizable" for x in missing])
        result.missing_factors.extend([f"{x}: bloqueado por conflicto de fuentes" for x in blocked])
        if confidence_result:
            result.red_flags.append(f"Calidad evidencia: {confidence_result.data_quality:.1f}% | Cobertura utilizable: {confidence_result.coverage:.1f}%")
        result.red_flags=list(dict.fromkeys(result.red_flags))
        context.decision=result
        metadata["confidence_result"]={"coverage":evidence_coverage,"data_quality":confidence_result.data_quality if confidence_result else 80.0,"confidence":evidence_confidence,"blocked":blocked,"missing":missing}
        return result
